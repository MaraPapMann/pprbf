...
/**
 * Aggregate two vectors accordingly to generate the first vector.
 * @param: vec_a: vector<vector<uint32_t>>; first input 2D vector(data from 1st hospital).
 * @param: vec_b: vector<vector<uint32_t>>; second input 2D vector(data from 2nd hospital).
 * return: vec_1st: vector<uint32_t>; aggregated vector.
 */
vector<uint32_t> aggregate_vector_1st_2(vector<vector<uint32_t>> vec_a, vector<vector<uint32_t>> vec_b){
		int i = vec_a.size();  // Row number of vector a.
		int j = vec_a[0].size();  // Column number of vector b.
		int k = vec_b.size();  // Row number of vector b.
		vector<uint32_t> vec_1st;  // Initialize
		for (vector<uint32_t> vec:vec_a)
		{
			for (int l = 0; l < k; l++)
			{
				vec_1st.insert(vec_1st.end(), vec.begin(), vec.end());  // Duplicate current vector times according to the row number of vector b.
			}
		}
		return vec_1st;
}
...
/**
 * Aggregate two vectors accordingly to generate the second vector.
 * @param: vec_a: vector<vector<uint32_t>>; first input 2D vector(data from 1st hospital).
 * @param: vec_b: vector<vector<uint32_t>>; second input 2D vector(data from 2nd hospital).
 * return: vec_2nd: vector<uint32_t>; aggregated vector.
 */
vector<uint32_t> aggregate_vector_2nd_2(vector<vector<uint32_t>> vec_a, vector<vector<uint32_t>> vec_b){
		int i = vec_a.size();  // Row number of vector a.
		int j = vec_a[0].size();  // Column number of vector b.
		int k = vec_b.size();  // Row number of vector b.
		vector<uint32_t> vec_2nd;  // Initializing
		for (int l = 0; l < i; l++)
		{
			for (vector<uint32_t> vec:vec_b)
			{
				vec_2nd.insert(vec_2nd.end(), vec.begin(), vec.end());  // Duplicate all vectors in this 2D vector times according to the row number of vector a.
			}
		}
		return vec_2nd;
}
...
// Test generate simulated data function
...
// Test generate long vector function
