...
uint32_t** csv_parser::get_data_as_array(){
	ifstream file(file_name);
    string line;
	uint32_t** data_arr;
	data_arr = new uint32_t*[nrow];
	for(int i=0; i<nrow; i++){
		data_arr[i] = new uint32_t[ncol];
	}
...
/* Tested! 
int main(){
	csv_parser csv_a("/home/chen/Git_repositories/pprbf/src/data/probe/a_encoded_test_seq.txt_split_1.csv", ',');
